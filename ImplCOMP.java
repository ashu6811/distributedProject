...
//public static final Logger LOGGER = Logger.getLogger(ImplCOMP.class.getName());
	...
	public static int threadFlag2 = 0;
	...
	public ImplCOMP(){
		...
		inner1.put("COMP6231", CAPACITY);
		inner1.put("COMP6241", CAPACITY);
		//inner1.put("COMP6251", CAPACITY);
		
		inner2.put("COMP6231", CAPACITY);
		inner2.put("COMP6241",CAPACITY);
		//inner2.put("COMP6251", CAPACITY);
		
		inner3.put("COMP6231",CAPACITY);
		inner3.put("COMP6241", CAPACITY);
		//inner3.put("COMP6251", CAPACITY);
...
//		LOGGER.info("Fresh courses are online to be registered (From Constructor)");       
		...
		clientsOuter.put("COMPS1111", clientsInner1);
		clientsOuter.put("COMPS2222", clientsInner2);
		clientsOuter.put("COMPS3333", clientsInner3);
		
		advisorList = new String[2];//not more than two advisor for one department
		advisorList[0] = "COMPA1111";
		advisorList[1] = "COMPA2222";
...
//					LOGGER.info("Course DataStructure exists (From printmap)");
	...
	public String displayCourseDetails()
	{
		...
		tempString2 = sendMessageToDisplay(2222);
...
//			LOGGER.info("Course Schedule for client "+studentID+" (From displayStudentDetailsP)");
		 ...
		 String[] tempArrayFall = clientsOuter.get(studentID).get("FALL");
       	 ...
       	 String retStr = "The class schedule of student "+studentID+" are: \n"+"Fall: = ";
       	 int i = 0;
...
//				LOGGER.info("Course already registered (from enrolCourse)");
				tempReturn =  "Operation failed! The Student "+ studentID +" is already enrolled into this course in "+semester;
			}
			else
			{
//				LOGGER.info("Course " + courseID + ", " + semester + " semester for student " + studentID + " is going to be registered (from enrolCourse)");
				...
				{
//					LOGGER.info("Course registration is going to happen within the same server COMP  (from enrolCourse)");
...
//						LOGGER.info("MAXCOUNT satisfied (from enrolCourse)");
						...
						for(int i=0; i < 3; i++)
						{
							...
							{
								tempReturn = "Operation failed! The student "+studentID+" is already enrolled in 3 courses in "+semester;
							}
						...
						tempReturn = "Operation Successfull! The student "+studentID+" is successfully enrolled in the course "+courseID+" in "+ semester;
//						LOGGER.info("Successfully added (from enrolCourse)");
						
					}
					else
					{
						tempReturn = "Operation failed! The course capacity is full";
					}
...
//					LOGGER.info("Course registeration is going to happen within the DIFFERENT server  (from enrolCourse)");
					
					
					if (tempSubString.compareTo("SOEN") == 0)
					{
						tempReturn = sendToEnrol(courseID, semester, 2223);
...
//						
					...
					{
//						LOGGER.info("COURSE NOT REGISTERED. Enter Correct Couse id.  (from enrolCourse)");
...
//						LOGGER.info("Course " + courseID + " for " + semester + " semester is registered by "+studentID+" and is ready to be dropped (from dropCourse)");
						...
						if (tempSubString.compareTo("COMP") == 0)
						{
//							LOGGER.info("Course Drop is going to happen within the same server COMP (from dropCourse)");
...
//										LOGGER.info("Course " + courseID + " for " + semester + " semeter by "+studentID+" is dropped (from dropCourse for loop)");
										...
										tempReturn = "Operation Successful! The student "+ studentID +" has removed from the course "+ courseID+ " in "+semester;
//										LOGGER.info("Successfully Dropped (from dropCourse)");
								...
								if (tempFlag == 0)
								{
									tempReturn = "Operation failed! The student "+studentID+" is not enrolled in this class "+courseID;
...
//											
											tempReturn = "Operation Successful! The student "+ studentID + sendToDrop(courseID,semester,2224);
//											LOGGER.info("Successfully Dropped (from dropCourse)");
											tempFlag = 1;
											break;
										}
...
//												
												tempReturn ="Operation Successful! The student "+ studentID + sendToDrop(courseID,semester,3335);
//												LOGGER.info("Successfully Dropped (from dropCourse)");
												tempFlag = 1;
												break;
											}
...
//						LOGGER.info("Course " + courseID + ", " + semester + " semeter for student " + studentID + " CANNOT be dropped (from dropCourse)");
						tempReturn = "Operation failed! The Course does not exist";
					}
...
//						LOGGER.info("Course " + courseID + " for " + semester + " semeter by "+ key +" is removed because of removal of course from the system  (from removeCourse)");
			...
			sendToRemove(courseID, semester, 2226);
...
//			LOGGER.info("Course " + courseID + " Successfully removed from the system for semester" + semester + "(from removeCourse)");
			tempReturn = "Operation successfull! The Course "+ courseID +" is removed for " + semester;
			
		}
		else
		{
			
			tempReturn = "Operation failed! The Course "+ courseID  +" is not existing in this "+semester;
		}
	...
	public static void displayReceive() {
		...
		try {
			...
			byte[] buffer = new byte[1000];
			System.out.println("Server 1111 COMP Started............");
			while (true) {
				String tempReturn ;
				...
				aSocket.receive(request);
					...
					DatagramPacket reply = new DatagramPacket(message1, message1.length, request.getAddress(),
							request.getPort());
					aSocket.send(reply);
					System.out.println("Display Outer  of COMP" + tempReturn);
	...
	try {
		...
		byte[] buffer = new byte[1000];
		System.out.println("Server 1112 COMP Started for Enrollment............");
		while (true) {
			String tempReturn ;
	...
	//14
		...
		public static void dropReceive() {
			...
			try {
				...
				aSocket = new DatagramSocket(1113);
				byte[] buffer = new byte[1000];
				System.out.println("Server 1113 COMP Started for Course Drop............");
				while (true) {
					String tempReturn ;
					String  courseID, semester;
					DatagramPacket request = new DatagramPacket(buffer, buffer.length);
					...
					outer.get(semester).put(courseID,tempValue);
						...
						tempReturn =  " has removed from the course "+ courseID+ " in "+semester;
						System.out.println("tempReturn " + tempReturn);
						byte[] message1 = tempReturn.getBytes();
						DatagramPacket reply = new DatagramPacket(message1, message1.length, request.getAddress(),
								request.getPort());
	...
	//swap function
		...
		public String swapCourse(String studentID,String newCourseID, String oldCourseID)
		{
			...
			if (studentVerification(studentID)==true)
			{
				...
				{
					retStr = "Operation failed! The Course does not exist";
				}
			}
			...
			if (checkStudentRegisteredCourses(studentID, newCourseID))
			{
				...
				retStr =  "Operation failed! The student "+studentID+" is already enrolled in this course "+newCourseID;
			}
			else
			{
				...
				if (tempSubString.compareTo("COMP") == 0)
				{
					...
					{
						retStr = "Operation failed! The capacity of the new course "+newCourseID+" is full";
					}
				}
					// course registration is in the ANOTHER server
						
				else if (tempSubString.compareTo("SOEN") == 0)
				{
					String tempReturn = sendToCheck(newCourseID, semester, 2225);
			...
			if (flag1 == 1 && flag2 == 1)
			{
				...
				retStr = "Swapping successful! The courses have been swapped successfully";
			}
			else
			{
				if (newCourseID.compareTo(oldCourseID)==0)
				{
					retStr = "Swapping Failed! Cannot Swap same courses";
				}
			}
			threadFlag2 =0;
			return retStr;
	...
	// 18 receive to check 
		public static void checkReceive() {
			...
			try {
				...
				aSocket = new DatagramSocket(1114);
				byte[] buffer = new byte[1000];
				System.out.println("Server 1114 COMP Started for checking............");
				while (true) {
					String tempReturn ;
					String  courseID, semester;
					DatagramPacket request = new DatagramPacket(buffer, buffer.length);
...
//20 receive remove
		...
		public static void removeReceive() {
			...
			try {
				...
				aSocket = new DatagramSocket(1115);
				byte[] buffer = new byte[1000];
				System.out.println("Server 1115 COMP Started for Course Remove............");
				while (true) {
					String tempReturn ;
					String  courseID, semester;
					DatagramPacket request = new DatagramPacket(buffer, buffer.length);
...
// 21 
		public static void mainReceive() {
			...
			try {
				...
				aSocket = new DatagramSocket(1116);
				byte[] buffer = new byte[1000];
				System.out.println("Server 1116 main COMP Started for invocation............");
				while (true) {
					String tempReturn ;
					String  courseID, semester;
					DatagramPacket request = new DatagramPacket(buffer, buffer.length);
...
//								LOGGER.info("Course " + courseID + " for " + semester + " semeter by "+ key +" is removed because of removal of course from the system  (from removeCourse)");
	...
	//21
		...
		public static void receiveFromRM() {
			DatagramSocket aSocket = null;
			try {
				ServerCOMP s = new ServerCOMP();
				aSocket = new DatagramSocket(1110);
				byte[] buffer = new byte[1000];
				System.out.println("Server 1110 COMP Started for invocation from RM...");
				while (true) {
					...
					System.out.println(breakMsg[0]);
					System.out.println(breakMsg[1]);
...
//					}
					...
					else
					{
						...
						DatagramPacket reply = new DatagramPacket(message1, message1.length, request.getAddress(),request.getPort());
						aSocket.send(reply);
						System.out.println("INVOKED AND message sent by COMP is: " + replyStr);
