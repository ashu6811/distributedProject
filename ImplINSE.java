...
//public static final Logger LOGGER = Logger.getLogger(ImplCOMP.class.getName());
	...
	public ImplINSE(){
		...
		inner1.put("INSE6231", CAPACITY);
		inner1.put("INSE6241", CAPACITY);
		//inner1.put("INSE6251", CAPACITY);
		
		inner2.put("INSE6231", CAPACITY);
		inner2.put("INSE6241", CAPACITY);
		//inner2.put("INSE6251", CAPACITY);
		
		inner3.put("INSE6231", CAPACITY);
		inner3.put("INSE6241", CAPACITY);
		//inner3.put("INSE6251", CAPACITY);
...
//		LOGGER.info("Fresh courses are online to be registered (From Constructor)");       
		...
		clientsOuter.put("INSES1111", clientsInner1);
		clientsOuter.put("INSES2222", clientsInner2);
		clientsOuter.put("INSES3333", clientsInner3);
		
		advisorList = new String[2];//not more than two advisor for one department
		advisorList[0] = "INSEA1111";
		advisorList[1] = "INSEA2222";
...
//					LOGGER.info("Course DataStructure exists (From printmap)");
	...
	public String displayCourseDetails()
	{
		...
		tempString3 = sendMessageToDisplay(1111);
...
//						LOGGER.info("COURSE NOT REGISTERED. Enter Correct Couse id.  (from enrolCourse)");
						tempReturn = "COURSE NOT REGISTERED. Enter Correct Couse id.";
					}
...
//											
											tempReturn = sendToDrop(courseID,semester,2224);
//											LOGGER.info("Successfully Dropped (from dropCourse)");
											tempFlag = 1;
											break;
										}
...
//											LOGGER.info("Deletion Unsucessfull(from deleteCourse tempFlag)");
					...
					{
//						LOGGER.info("Course " + courseID + ", " + semester + " semester for student " + studentID + " CANNOT be dropped (from dropCourse)");
...
//			LOGGER.info("Course " + courseID + " Successfully removed from the system for semester" + semester + "(from removeCourse)");
	...
	public static void displayReceive() {
		...
		try {
			...
			byte[] buffer = new byte[1000];
			System.out.println("Server 3333 INSE Started............");
			while (true) {
				String tempReturn ;
				...
				aSocket.receive(request);
					...
					DatagramPacket reply = new DatagramPacket(message1, message1.length, request.getAddress(),
							request.getPort());
					aSocket.send(reply);
					System.out.println("Display Outer  of INSE" + tempReturn);
	...
	try {
		...
		byte[] buffer = new byte[1000];
		System.out.println("Server 3334 INSE Started for Enrollment............");
		while (true) {
			String tempReturn ;
			...
			else
			{
				...
				System.out.println("Enrolled and message sent by INSE is: " + tempReturn);
	...
	//14
		...
		public static void dropReceive() {
			...
			try {
				...
				aSocket = new DatagramSocket(3335);
				byte[] buffer = new byte[1000];
				System.out.println("Server 3335 INSE Started for Course Drop............");
				while (true) {
					String tempReturn ;
					String  courseID, semester;
					DatagramPacket request = new DatagramPacket(buffer, buffer.length);
					...
					outer.get(semester).put(courseID,tempValue);
						...
						DatagramPacket reply = new DatagramPacket(message1, message1.length, request.getAddress(),
								request.getPort());
						aSocket.send(reply);
						System.out.println("Dropped and message sent by INSE is: " + tempReturn);
		...
		// 18 receive to check 
			public static void checkReceive() {
				...
				try {
					...
					aSocket = new DatagramSocket(3336);
					byte[] buffer = new byte[1000];
					System.out.println("Server 3336 INSE Started for checking............");
					while (true) {
						String tempReturn ;
						String  courseID, semester;
						DatagramPacket request = new DatagramPacket(buffer, buffer.length);
			...
			//20 receive remove
					...
					public static void removeReceive() {
						...
						try {
							...
							aSocket = new DatagramSocket(3337);
							byte[] buffer = new byte[1000];
							System.out.println("Server 3337 INSE Started for Course Remove............");
							while (true) {
								String tempReturn ;
								String  courseID, semester;
								DatagramPacket request = new DatagramPacket(buffer, buffer.length);
...
//											LOGGER.info("Course " + courseID + " for " + semester + " semeter by "+ key +" is removed because of removal of course from the system  (from removeCourse)");
					...
					public static void receiveFromRM() {
						DatagramSocket aSocket = null;
						try {
							ServerINSE s = new ServerINSE();
							aSocket = new DatagramSocket(3330);
							byte[] buffer = new byte[1000];
							System.out.println("Server 3330 INSE Started for invocation from RM...");
							while (true) {
